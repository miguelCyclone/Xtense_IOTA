...
function findAllSeeds (userID){
	...
	for (var a=0; a<user.seedArray.length;a++){
		console.log("const seed"+user.userID+"_"+user.seedArray[a].asset+" = '"+user.seedArray[a].seed+"';");
...
function initUser (userID){
	...
	if (findUser(userID) == false){
	   ...
	   for (var a=0; a<assetArray.length; a++){
		  ...
		  if (a==2) {n='C9';}
		  if (a==3) {n='D9';}
		  var auxSeed = 'PUEOTSEITFEVEWCWBTSIZM9NKRGJEIMXTULBACGFRQK9IMGICLBKW9TTEVSDQMGWKBXPVCBMMCXS'+n+userID;
	...
	} else {console.log("User "+ userID + " already exists");}	
}	
...
const addAsset = async (assetID, name, ammount, ISIN) => {
	...
	if (findAsset(assetID) == false){
	  ...
	  for (var a=0; a<userArray.length; a++){
		...
		if (a==2) {n='C9';}
		if (a==3) {n='D9';}
		var auxSeed = 'PUEOTSEITFEVEWCWBTSIZM9NKRGJEIMXTULBACGFRQK9IMGICLBKW9TTEVSDQMGWKBXPVCBMMCXS'+n+assetID;
		//END Only for testing	
	...
	} else {console.log("Asset "+ assetID + " already exists");}	
}
...
const transferShares = async (userTxID, userRxID, assetID, ammount) => {
  ...
  console.log(transfers)
   ...
   try {
	...
	console.log(userTx.userID+' sent  '+ammount+" shares to "+userRx.userID+" at address: "+"\n"+"'"+receivingAddress+"'")
	return response
  } catch (e) {
    console.log(e)
	ar.push(0)
...
const checkAssetBalance = async (assetID) => {
	...
	if (key ==1){
		console.log("BALANCE IS: "+balance+" token shares from asset "+asset.assetID)
		ar.push(1)
	...
	} catch (e) {console.log(e);ar.push(e);}
}
...
var userBalanceInAsset = async (userID, assetID) => {
	try{
        ...
        if(seed != false){
           ...
           console.log('   User "'+userID+'" '+'balance for asset "'+assetID+'"is :'+total.totalBalance)
	       return total.totalBalance
        }else{console.log("ERROR USER BALANCE: FindSeed function didn't find seed");}
	}catch (e){console.log(e)}
}
...
const sharesDistribution = async (assetID) => {
	...
	if (asset != false){
		...
		for(var a=0; a < userArray.length; a++){
			...
			aux.balance = await userBalanceInAsset(userArray[a].userID, asset.assetID)
...
//Export functions
module.exports = {init, findAllSeeds, checkAssetBalance, transferShares, userArray, assetArray, addressBalance, userBalanceInAsset, userTotalBalance, sharesDistribution};
